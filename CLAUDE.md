...
- **アクセス計測**: GA4（`VITE_GA_MEASUREMENT_ID`）。`src/lib/analytics.ts` の `useAnalytics()` を `App.tsx` で呼び、SPA のルート遷移ごとに `page_view` を送信（本番ビルドのみ有効・dev は無効）。手順は `docs/ga4-setup.md`
