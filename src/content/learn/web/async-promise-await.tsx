...
export default function Article() {
  return (
    <>
      ...
      <Section>「遅い」の正体 — 時間はどこに消えているか</Section>
      <p>
        非同期を学ぶ前に「そもそも、なぜ遅いのか」を掴んでおくと一気に腑に落ちます。Web アプリの遅さの大半は、<strong>計算（CPU）ではなく待ち時間（I/O）</strong>です。API 通信や DB アクセスは、頼んでから結果が返るまで<strong>ネットワークを往復する時間</strong>がかかります。CPU はその間、ただ返事を待っているだけです。
      </p>
      <ComparisonTable
        headers={["処理の種類", "だいたいの時間", "体感"]}
        rows={[
          ["CPU での計算（メモリ読み書き）", "ナノ〜マイクロ秒", "一瞬"],
          ["SSD からの読み込み", "数十マイクロ秒", "ほぼ一瞬"],
          ["同一データセンター内の通信", "1 ミリ秒未満", "速い"],
          ["API / DB への往復（ネット経由）", "数十〜数百ミリ秒", "「待ち」を感じる"],
          ["遠いサーバ・低速回線", "数百ミリ秒〜数秒", "明確に遅い"],
        ]}
      />
      <p>
        つまり遅さの正体は「CPU がサボっているのではなく、<strong>ネットワークやディスクからの返事を待っている</strong>」時間です。ここで<strong>同期（ブロッキング）</strong>で書くと、待っている間ずっと画面が固まります。<strong>非同期</strong>にすると、その待ち時間を別の作業に回せるので画面が固まりません。「非同期にすると速くなる」のではなく、正確には<strong>「待ち時間を無駄にしなくなる」</strong>のです。
      </p>
