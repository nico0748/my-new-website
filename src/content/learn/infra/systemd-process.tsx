...
import { Lead, Section, Callout, Code, Cmd, KeyPoints, Figure, Divider } from "../../../components/learn/kit";
...
systemctl disable <service>                   # 自動起動を無効化`}</Code>
      <Figure
        src="/learn/shots/infra/systemd-process-01.svg"
        alt="systemctl status ssh の実行結果で active (running) が緑色で表示されているターミナル"
        caption="systemctl status の出力。Loaded / Active / 直近のログまで1画面で読めるのが systemd の便利なところ。"
      />
...
ss -tlnp                         # どのプロセスがどのポートで待ち受けているか`}</Code>
      <Figure
        src="/learn/shots/infra/systemd-process-02.svg"
        alt="htop の画面。上部に CPU とメモリの使用率バー、下部にプロセス一覧が表示されている"
        caption="htop の画面。上のバーで CPU・メモリの逼迫を一目で掴み、下の一覧で犯人のプロセスを探す。"
      />
...
sudo grep "Failed\\|Invalid" /var/log/auth.log | wc -l`}</Code>
      <Figure
        src="/learn/shots/infra/systemd-process-03.svg"
        alt="auth.log を grep した結果、Failed password や Invalid user のログが大量に並んでいるターミナル"
        caption="立てて数時間の VPS でもこの量。世界中のボットが常時 SSH を叩きに来ているのが実際に見える。"
      />
