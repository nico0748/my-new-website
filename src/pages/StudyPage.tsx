...
const StudyPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (rows.length > 0) setStudies(mapStudyData(rows));
      } catch {
        // fallback
      }
      setLoading(false);
  ...
  const categories = useMemo(() => {
    const set = new Set(studies.map((s) => s.category.trim().toLowerCase()));
  ...
  const filtered = useMemo(
    () =>
      activeCategory === "all"
        ? studies
        : studies.filter((s) => s.category.trim().toLowerCase() === activeCategory),
      ...
      <div className="font-sans antialiased min-h-screen">
        ...
        <main className="max-w-7xl mx-auto px-4 py-12">
          ...
          >
            ...
            >
              <span style={{ color: "var(--accent)" }}>~/</span>study
          ...
          {categories.length > 1 && (
            <div className="flex flex-wrap justify-center gap-2 mb-10">
              {["all", ...categories].map((c) => {
                ...
                const label = c === "all" ? "All" : getStudyCategoryStyle(c).label;
                return (
                  <button
                    key={c}
