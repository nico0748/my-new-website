...
const TopicDetailPage = () => {
    ...
    const load = async () => {
      ...
      if (import.meta.env.VITE_GOOGLE_SHEETS_API_KEY) {
        try {
          ...
          if (rows.length > 0) items = mapTopicData(rows);
        } catch {
          // fallback
        }
      }
      ...
      const mdPath = found?.markdownFile ?? `/topics/${id}.md`;
      try {
  ...
  const cat = topic ? getTopicCategoryStyle(topic.category) : null;
  // md が無い場合でも、原典リンク or 説明があればフォールバック表示する
  const hasFallback = mdError && !!topic && (!!topic.externalUrl || !!topic.description);
            ...
            {mdError && !hasFallback ? (
              ...
              <div className="prose-custom">
                ...
                {topic?.externalUrl && (
                  <p>
                    詳細は{" "}
                    <a href={topic.externalUrl} target="_blank" rel="noopener noreferrer">
                      こちらの原典リンク
                    </a>{" "}
                    を参照してください。
                  </p>
